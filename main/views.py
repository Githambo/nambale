...
class GiftAllocation(LoginRequiredMixin, CreateView):
    ...
    def get_initial(self):
    	initial=super().get_initial()
    	initial['student']= Student.objects.get(pk=self.request.GET['student'])
    	return initial
...
def student_download(request):	
	...
	writer.writerow(['NAME','LOCATION','CLASS','TALENT','PARENT','STATUS','VILLAGE','TEACHER','CDC NAME','CHURCH'])

	for rows in report:
		writer.writerow([rows,rows.location,rows.level,rows.talent,rows.parent,rows.sponsor_status,rows.village,rows.teacher,rows.centre,rows.sponsor])
